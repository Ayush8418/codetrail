...
type Subject = { _id: string; name: string; notesCount: number; createdAt: string };

export default function OldNotePage() {
  ...
  const fetchSubjects = async () => {
    try {
      ...
      setSubjects((data.data || []).sort((a: Subject, b: Subject) => b.notesCount - a.notesCount));
    } catch (err: any) {
  ...
  const handleDelete = async (name: string) => {
    ...
    try {
      setDeleting(name);
      const res = await fetch(`/api/notesubject?subject=${encodeURIComponent(name)}`, { method: "DELETE" });
      const data = await res.json();
      if (!res.ok) throw new Error(data.message);
  ...
  return (
    <div className="min-h-screen px-8 py-10 bg-white dark:bg-black transition-transform duration-200 hover:-translate-y-0.5">

      {/* Header */}
      <div className="flex items-center justify-between mb-10">
        <div>
          <h1 className="text-2xl font-semibold text-zinc-900 dark:text-zinc-100">Notes</h1>
      ...
      {/* Grid */}
      {subjects.length === 0 && (
        <p className="text-sm text-zinc-400">No subjects yet.</p>
      )}

      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {subjects.map((subject) => (
          <div key={subject._id} className="relative group rounded-2xl overflow-hidden">

            {/* Blurry hazy background */}
            <div className="absolute inset-0 bg-zinc-100 dark:bg-zinc-900 backdrop-blur-xl" />
            <div className="absolute inset-0 bg-gradient-to-br from-white/60 via-zinc-100/40 to-zinc-200/60 dark:from-zinc-800/60 dark:via-zinc-900/40 dark:to-black/60" />
            <div className="absolute inset-0 border border-zinc-200 dark:border-zinc-800 rounded-2xl" />

            <Link href={`/note/${encodeURIComponent(subject.name)}`} className="relative block px-5 py-5">
