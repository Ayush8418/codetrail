...
export default function OldNotePage() {
  ...
  const sortedSubjects = [...subjects].sort((a, b) => {
    ...
    if (sortBy === "least-notes") {
      return a.notesCount - b.notesCount;
    }
    return 0;
  });
  ...
  const handleDeleteSubject = async (name: string) => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/notesubject?subject=${encodeURIComponent(name)}`,
        { method: "DELETE" }
      );
    ...
    <div className="p-6 bg-gray-50 dark:bg-zinc-950 min-h-screen">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            <option value="least-notes">Least notes</option>
          </select>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {sortedSubjects.map((subject) => (
          ...
          >
            <Link
              href={`/note/oldnotes/${encodeURIComponent(subject.name)}`}
              className="block"
            >
